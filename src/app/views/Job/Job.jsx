...
import { searchByPage, deleteItem, getItemById } from "./JobService";
...
function MaterialButtonListCv(props) {
    const { t, i18n } = useTranslation();
    const item = props.item;
    return (
        <div className="none_wrap">
            <LightTooltip
                title={t("general.viewApply")}
                placement="right-end"
                enterDelay={300}
                leaveDelay={200}
            >
...
class Job extends React.Component {
    ...
    render() {
        ...
        let columns = [
                            ...
                            if (method === ConstantList.actionMethod.edit) {
                                if (rowData.id) {
                                    getItemById(rowData.id).then(({ data }) => {
                                        ...
                                        toast.error(t("general.getInforError"))
                                    })
                                }
            ...
            {
                title: t("general.dateSubmitted"),
                field: "createDate",
                align: "center",
                minWidth: 250,
                maxWidth: 250,
                render: (rowData) => {
                    return `${rowData.createDate[2]}-${rowData.createDate[1]}-${rowData.createDate[0]} ${rowData.createDate[3]}:${rowData.createDate[4]}:${rowData.createDate[5]}`
                },
                headerStyle: {
                    ...ConstantList.styleTable.columnFirst
                },
            ...
            },
            {
                ...
                render: (rowData) => {
                    return rowData.bestJob ? <span>{t("Yes")}</span> : <span>{t("No")}</span>
                }
            }
        ];
        return (
            <div className="m-50">
                <div className="mb-sm-30 pb-16" style={{ borderBottom: '1px solid #ccc' }}>
                    <Breadcrumb
                        routeSegments={[{ name: t("Dashboard.job") }]}
                    />
                </div>
                    ...
                    <CardHeader
                        ...
                        action={
                            <Grid container className="space-between">
                                ...
                                {this.state.shouldOpenJobCvDialog && (
                                    <JobCv
                                        t={t}
                                        i18n={i18n}
                                        handleClose={this.handleDialogClose}
                                        open={this.state.shouldOpenJobCvDialog}
