...
import { addFile, addInternalCertificate, getFile, getInternalCertificateByIdSailor, getNationalUnit, removeInternalCertificate, searchByPage } from "../SailorService";
...
function InternalCertificate(props) {
  ...
  const getDataList = (id) => {
    getInternalCertificateByIdSailor(id)
      .then(({ data }) => {
  ...
  const handleConfirmDelete = () => {
    if (certificate.itemList[idDelete]?.id) {
      removeInternalCertificate(certificate.itemList[idDelete].id)
        .then(({ data }) => {
  ...
  const handleSubmit = () => {
    setLoading(true)
    addInternalCertificate(certificate.itemList, dataSailor.id)
      .then(({ data }) => {
        if (data.code === 200) {
