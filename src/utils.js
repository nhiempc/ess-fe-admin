...
// Refactor
export const handleDeleteAll = async (deleteFunc, list) => {
  let listAlert = [];
  for (let i = 0; i < list.length; i++) {
      await deleteFunc(list[i].id)
          .then(({ data }) => {
              if (data.code !== 200) {
                  listAlert.push(list[i]);
              }
          })
  }
  if (listAlert.length === list.length) {
      toast.warning("Danh mục đã được sử dụng");
  } else if (listAlert.length > 0) {
      toast.warning("Đã xoá các danh mục chưa sử dụng");
  } else {
